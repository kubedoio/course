...
install KUBEADM from kubernertes repo..
...
First step is to initialize the cluster in the first terminal:
  ...
  ```.term1
  kubeadm init --apiserver-advertise-address $(hostname -i) --ignore-preflight-errors all
  ```

(Preflight errors are ingored to prevent our exceptions)
